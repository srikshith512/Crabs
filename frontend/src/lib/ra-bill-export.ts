...
function buildAbstractWorksheet(bill: StoredRABill) {
  const rows: string[] = [];
  ...
  return worksheet("Abstract", rows);
}

function buildMeasurementWorksheet(bill: StoredRABill) {
  const rows: string[] = [];
  ...
  if (measurementItems.length === 0) {
    ...
    return worksheet("Measurement Summary", rows);
  }
...
export function exportRABillToExcel(bill: StoredRABill) {
  const workbook = `<?xml version="1.0"?>
    ...
    <Workbook
      ...
      ${buildAbstractWorksheet(bill)}
      ${buildMeasurementWorksheet(bill)}
    </Workbook>`;
