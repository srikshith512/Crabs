...
export function RABillDialog({ project, order, items, onGenerated }: Props) {
  ...
  const { billLines, lockCandidates, billItems, totals } = useMemo(() => {
    const lines: BillLine[] = [];
    const candidates: LockCandidate[] = [];

    items.forEach((item) => {
      const usableRows = (item.measurements || []).filter(hasPositiveMeasurement);
      ...
      (item.milestones || []).forEach((milestone) => {
        ...
        usableRows.forEach((measurement) => {
        ...
        const previousAmount = previousQty * Number(item.rate || 0);
        const currentAmount = currentQty * Number(item.rate || 0);
  ...
  const handlePrint = async () => {
    ...
    try {
      await generateSegmentedBillPDF(["bill-abstract-pdf-section"], {
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-6 py-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-800 font-bold rounded-2xl transition-all border border-slate-200 dark:border-slate-800 shadow-sm active:scale-95"
      >
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-8 right-8 z-[80] min-w-[320px] max-w-[420px] overflow-hidden rounded-3xl border border-slate-200 bg-white p-4 shadow-[0_20px_50px_rgba(0,0,0,0.15)]"
          >
            ...
            <div className="flex items-center gap-4 pl-2">
              ...
              <div>
                <p className="text-base font-black text-slate-900">{toast.title}</p>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-3 print:static print:bg-white print:p-0">
          <div className="relative flex h-[calc(100vh-2rem)] w-full max-w-[calc(100vw-1rem)] flex-col overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-2xl print:h-auto print:max-w-none print:rounded-none print:border-0 print:bg-white print:shadow-none">
            <div className="flex items-start justify-between gap-4 bg-slate-100 px-8 pb-4 pt-7 print:hidden">
              <div>
                <h2 className="text-2xl font-black tracking-tight text-slate-900">Abstract Sheet - {nextRANumber}</h2>
              ...
              <button
                ...
                className="rounded-lg p-1.5 text-slate-500 transition-all hover:text-slate-900"
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-8 pb-48 print:overflow-visible print:p-0">
              ...
              <div id="ra-bill-print-area" className="text-slate-900 print:bg-white">
                <style jsx global>{`
                  @media print {
                    body * {
                      visibility: hidden;
                ...
                <section id="bill-abstract-section" className="ra-print-section">
                  <div className="mb-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="grid grid-cols-1 gap-x-16 gap-y-7 md:grid-cols-2">
                      <div>
                        ...
                        <p className="text-base font-black text-slate-900">{project?.name || "Project"}</p>
                      </div>
                      <div>
                        <p className="text-base font-medium text-slate-500">Client:</p>
                        <p className="text-base font-black text-slate-900">{project?.client_name || "-"}</p>
                      </div>
                      <div>
                        <p className="text-base font-medium text-slate-500">Order Number:</p>
                        <p className="text-base font-black text-slate-900">{order?.order_code || "---"}</p>
                      </div>
                      <div>
                    ...
                    <table className="ra-table w-full border-collapse">
                      <thead>
                        <tr className="bg-slate-100">
                          <th rowSpan={2} className="whitespace-nowrap border border-slate-200 px-3 py-3 text-left text-sm font-black">PO Sr No</th>
                          <th rowSpan={2} className="whitespace-nowrap border border-slate-200 px-3 py-3 text-left text-sm font-black">ITEM CODE</th>
                          <th rowSpan={2} className="min-w-[220px] border border-slate-200 px-3 py-3 text-left text-sm font-black">Item Description</th>
                          ...
                          <th rowSpan={2} className="min-w-[210px] border border-slate-200 px-3 py-3 text-left text-sm font-black">BILL BREAK UP AS PER LOI</th>
                          <th rowSpan={2} className="whitespace-nowrap border border-slate-200 px-3 py-3 text-center text-sm font-black">Break up %</th>
                          <th rowSpan={2} className="whitespace-nowrap border border-slate-200 px-3 py-3 text-right text-sm font-black">Unit Rate</th>
                          ...
                          <th rowSpan={2} className="border border-slate-200 px-3 py-3 text-left text-sm font-black">REMARKS</th>
                        </tr>
                        <tr className="bg-slate-100">
                          <th className="whitespace-nowrap border border-slate-200 px-3 py-2 text-right text-sm font-black">Certified Qty</th>
                      ...
                      <tbody>
                        {billLines.length === 0 ? (
                          <tr>
                            <td colSpan={15} className="border border-slate-200 px-4 py-8 text-center font-bold text-slate-500">
                              No completed measurement quantities available for RA bill generation.
                            </td>
                          </tr>
                        ) : (
                          billLines.map((line, index) => (
                            <tr key={`${line.item.id}-${line.milestoneKey}`} className="bg-slate-50/40">
                              <td className="border border-slate-200 px-3 py-3 text-sm">{index + 1}</td>
                              <td className="border border-slate-200 px-3 py-3 text-sm">{line.item.item_code || "-"}</td>
                              <td className="border border-slate-200 px-3 py-3 text-sm">{line.item.description}</td>
                              <td className="border border-slate-200 px-3 py-3 text-center text-sm">{line.item.unit}</td>
                              <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatQty(Number(line.item.quantity || 0))}</td>
                              <td className="border border-slate-200 px-3 py-3 text-sm">{line.milestone.name}</td>
                              <td className="border border-slate-200 px-3 py-3 text-center text-sm">{line.milestone.percentage}%</td>
                              <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatMoney(Number(line.item.rate || 0))}</td>
                              ...
                              <td className="border border-slate-200 px-3 py-3 text-right text-sm font-bold">{formatQty(line.currentQty)}</td>
                              <td className="border border-slate-200 px-3 py-3 text-right text-sm font-bold">{formatMoney(line.currentAmount)}</td>
                              ...
                              <td className="border border-slate-200 px-3 py-3 text-sm"></td>
                            </tr>
                          ))
                        )}
                      </tbody>
                      <tfoot>
                        <tr className="bg-slate-100 font-black">
                          <td colSpan={8} className="border border-slate-200 px-3 py-3 text-left text-sm">TOTAL AMOUNT (INR)</td>
                          <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatQty(totals.previousQty)}</td>
                          <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatMoney(totals.previousAmount)}</td>
                          <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatQty(totals.currentQty)}</td>
                          <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatMoney(totals.currentAmount)}</td>
                          <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatQty(totals.cumulativeQty)}</td>
                          <td className="border border-slate-200 px-3 py-3 text-right text-sm">{formatMoney(totals.cumulativeAmount)}</td>
                  ...
                  <div className="mt-7 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
                    <h3 className="text-2xl font-black tracking-tight text-slate-900">Measurement Sheet Summary</h3>
                    <div className="mt-6 grid gap-4 lg:grid-cols-3">
                      <div className="rounded-2xl bg-slate-50 p-5">
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Items Covered</p>
                        <p className="mt-3 text-2xl font-black text-slate-900">{billItems.length}</p>
                      </div>
                      <div className="rounded-2xl bg-slate-50 p-5">
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">This Bill Quantity</p>
                        <p className="mt-3 text-2xl font-black text-slate-900">{formatQty(totals.currentQty)} {billLines[0]?.item.unit || "MT"}</p>
                      </div>
                ...
                <section id="bill-measurement-section" className="ra-page-break absolute left-[-10000px] top-0 w-[1400px] bg-white p-6">
                  ...
                  {billItems.length === 0 ? (
                    <div className="border border-slate-900 p-8 text-center font-bold text-slate-500">No measurement rows to display.</div>
                  ) : (
                      ...
                      return (
                        <div key={item.id} className={itemIndex > 0 ? "ra-print-section mt-8" : "ra-print-section"}>
                          ...
                          <div className="overflow-x-auto">
                            <table className="ra-table w-full border-collapse border border-slate-900">
                              <thead>
                                <tr className="bg-slate-100">
                                  <th className="border border-slate-900 p-2 text-center">Sr.</th>
                                  {columns.map(([label]) => (
                                    <th key={label} className="border border-slate-900 p-2 text-center">{label}</th>
                                  ))}
                                  <th className="border border-slate-900 p-2 text-right">Total ({item.unit})</th>
                                  {(item.milestones || []).map((milestone) => (
                                    <th key={milestone.name} className="border border-slate-900 p-2 text-center">
                              ...
                              <tbody>
                                {itemRows.map((measurement, rowIndex) => (
                                  <tr key={measurement.id}>
                                    <td className="border border-slate-900 p-2 text-center">{rowIndex + 1}</td>
                                    {columns.map(([label, key]) => (
                                      <td key={`${measurement.id}-${label}`} className="border border-slate-900 p-2 text-center">
                                        {getDisplayValue(measurement, key)}
                                      </td>
                                    ))}
                                    <td className="border border-slate-900 p-2 text-right font-bold">{formatQty(Number(measurement.quantity || 0))}</td>
                                    {(item.milestones || []).map((milestone) => {
                                      ...
                                      return (
                                        <td key={key} className="border border-slate-900 p-2 text-center font-bold">
                                          {qty > 0 ? formatQty(qty) : "-"}
                                        </td>
                                      );
                                    })}
                ...
                <div className="hidden">
                  <section id="bill-abstract-pdf-section" className="bg-white px-5 py-5 text-slate-900">
                    <div className="mb-4 grid grid-cols-2 gap-x-10 gap-y-5 rounded-[18px] border border-slate-300 bg-white p-4">
                      <div>
                        <p className="text-[11px] font-medium text-slate-500">Project:</p>
                        <p className="mt-1 text-[15px] font-black text-slate-900">{project?.name || "Project"}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-medium text-slate-500">Client:</p>
                        <p className="mt-1 text-[15px] font-black text-slate-900">{project?.client_name || "-"}</p>
                      </div>
                      <div>
                        <p className="text-[11px] font-medium text-slate-500">Order Number:</p>
                        <p className="mt-1 text-[15px] font-black text-slate-900">{order?.order_code || "---"}</p>
                      </div>
                      <div>
                    ...
                    <div className="mt-5 rounded-[18px] border border-slate-300 bg-slate-50 p-4">
                      <h3 className="text-[20px] font-black tracking-tight text-slate-900">Measurement Sheet Summary</h3>
                      <div className="mt-4 grid grid-cols-3 gap-4">
                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">Items Covered</p>
                          <p className="mt-2 text-[24px] font-black text-slate-900">{billItems.length}</p>
                        </div>
                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">This Bill Quantity</p>
                          <p className="mt-2 text-[24px] font-black text-slate-900">{formatQty(totals.currentQty)} {billLines[0]?.item.unit || "MT"}</p>
                        </div>
                        <div className="rounded-2xl border border-slate-200 bg-white p-4">
                          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">This Bill Amount</p>
                          <p className="mt-2 text-[24px] font-black text-slate-900">{formatMoney(totals.currentAmount)}</p>
                        </div>
                      </div>
                    </div>
                  </section>

                  <section id="bill-measurement-pdf-section" className="bg-white px-8 py-8 text-slate-900">
                    <div className="mb-6 flex items-center justify-between border-b border-slate-300 pb-4">
                      <div>
                        ...
                        <h3 className="mt-3 text-3xl font-black text-slate-900">Measurement Sheet Summary</h3>
                      </div>
                    ...
                    {billItems.length === 0 ? (
                      ...
                      billItems.map((item, itemIndex) => {
                        ...
                        return (
                          <div key={`pdf-measure-${item.id}`} className={itemIndex > 0 ? "mt-8 overflow-hidden rounded-[24px] border border-slate-900/80" : "overflow-hidden rounded-[24px] border border-slate-900/80"}>
                            ...
                            <table className="w-full border-collapse text-[11px]">
                              <thead>
                                <tr className="bg-slate-100">
                                  <th className="border border-slate-900/80 px-2 py-3 text-center font-black">Sr.</th>
                                  {columns.map(([label]) => (
                                    <th key={`pdf-col-${item.id}-${label}`} className="border border-slate-900/80 px-2 py-3 text-center font-black">{label}</th>
                                  ))}
                                  <th className="border border-slate-900/80 px-2 py-3 text-right font-black">Total ({item.unit})</th>
                                  {(item.milestones || []).map((milestone) => (
                                    <th key={`pdf-ms-${item.id}-${milestone.name}`} className="border border-slate-900/80 px-2 py-3 text-center font-black">
                                      {milestone.percentage}% {milestone.name}
                                    </th>
                                  ))}
                                </tr>
                              ...
                              <tbody>
                                {itemRows.map((measurement, rowIndex) => (
                                  <tr key={`pdf-row-${measurement.id}`}>
                                    <td className="border border-slate-900/80 px-2 py-2 text-center">{rowIndex + 1}</td>
                                    {columns.map(([label, key]) => (
                                      <td key={`pdf-cell-${measurement.id}-${label}`} className="border border-slate-900/80 px-2 py-2 text-center">
                                        {getDisplayValue(measurement, key)}
                                      </td>
                                    ))}
                                    <td className="border border-slate-900/80 px-2 py-2 text-right font-bold">{formatQty(Number(measurement.quantity || 0))}</td>
                                    {(item.milestones || []).map((milestone) => {
                                      const key = getMilestoneKey(item, milestone);
                                      const qty = lockCandidates
                                        .filter((candidate) => candidate.item.id === item.id && candidate.measurement.id === measurement.id && candidate.milestoneKey === key)
                                        .reduce((sum, candidate) => sum + candidate.qtyToLock, 0);
                                      return (
                                        <td key={`pdf-milestone-${measurement.id}-${key}`} className="border border-slate-900/80 px-2 py-2 text-center font-bold">
                                          {qty > 0 ? formatQty(qty) : "-"}
                                        </td>
                                      );
                                    })}
                                ...
                                <tr className="bg-slate-100">
                                  ...
                                  <td colSpan={Math.max(1, (item.milestones || []).length)} className="border border-slate-900/80 px-3 py-3 text-center text-lg font-black text-slate-900">
                    ...
                    <div className="mt-8 rounded-[24px] border border-slate-900/80 bg-slate-50 p-5">
                      ...
                      <div className="mt-4 flex flex-wrap gap-12 text-lg">
                        <p className="text-slate-500">
                          Total Weight: <span className="font-black text-slate-900">{formatQty(totals.currentQty)} {billLines[0]?.item.unit || "MT"}</span>
                        </p>
                        <p className="text-slate-500">
                          Bill Amount: <span className="font-black text-slate-900">{formatMoney(totals.currentAmount)}</span>
                        </p>
                      </div>
                    </div>
                  </section>
            ...
            <div className="absolute bottom-0 left-0 right-0 border-t border-slate-200 bg-slate-100 px-8 py-5 print:hidden">
              ...
              <div className="flex flex-wrap items-center justify-end gap-3">
                <button
                  ...
                  className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-5 py-3 text-base font-medium text-slate-900 shadow-sm transition-all hover:bg-slate-50"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-6 py-3 text-base font-medium text-slate-900 shadow-sm transition-all hover:bg-slate-50"
                >
