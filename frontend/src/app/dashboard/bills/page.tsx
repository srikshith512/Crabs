...
export default function BillHistoryPage() {
  ...
  const exportBillPdf = async (bill: StoredRABill) => {
    ...
    await generateSegmentedBillPDF(["history-bill-abstract", "history-bill-measurements"], {
        ...
        {selectedBill && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4">
            ...
            <motion.div
              ...
              className="relative z-10 flex h-[calc(100vh-2rem)] w-full max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-[30px] bg-slate-100 dark:bg-slate-900 shadow-2xl"
            >
              <div className="flex items-start justify-between gap-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-8 py-6">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.24em] text-blue-600">Bill Details</p>
                  <h2 className="mt-2 text-3xl font-black tracking-tight text-slate-900 dark:text-white">{selectedBill.raNumber}</h2>
                ...
                <button
                  onClick={() => setSelectedBill(null)}
                  className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-3 text-slate-500 transition-colors hover:text-slate-900 dark:text-white"
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto px-8 py-6">
                {isLoadingDetail ? (
                  <div className="flex h-full items-center justify-center">
                    <div className="flex items-center gap-3 rounded-2xl bg-white dark:bg-slate-950 px-5 py-4 text-sm font-bold text-slate-600 dark:text-slate-300 shadow-sm">
                  ...
                  <div>
                    <section id="history-bill-abstract" className="space-y-6">
                      <div className="rounded-[28px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 shadow-sm">
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                          <div>
                            ...
                            <p className="mt-2 text-base font-black text-slate-900 dark:text-white">{selectedBill.projectName}</p>
                          </div>
                          <div>
                            <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Client</p>
                            <p className="mt-2 text-base font-black text-slate-900 dark:text-white">{selectedBill.clientName}</p>
                          </div>
                          <div>
                            <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Order</p>
                            <p className="mt-2 text-base font-black text-slate-900 dark:text-white">{selectedBill.orderNumber}</p>
                          </div>
                          <div>
                            <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Generated On</p>
                            <p className="mt-2 text-base font-black text-slate-900 dark:text-white">{formatDateTime(selectedBill.generatedAt)}</p>
                          </div>
                        </div>
                      </div>

                      <div className="overflow-x-auto rounded-[28px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-sm">
                        <table className="w-full border-collapse">
                          <thead>
                            <tr className="bg-slate-100 dark:bg-slate-900">
                              <th rowSpan={2} className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-left text-sm font-black">PO Sr No</th>
                              <th rowSpan={2} className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-left text-sm font-black">Item Code</th>
                              <th rowSpan={2} className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-left text-sm font-black">Item Description</th>
                              ...
                              <th rowSpan={2} className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-left text-sm font-black">Bill Break Up</th>
                              <th rowSpan={2} className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-center text-sm font-black">Break up %</th>
                              <th rowSpan={2} className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-right text-sm font-black">Unit Rate</th>
                            ...
                            <tr className="bg-slate-100 dark:bg-slate-900">
                              <th className="border border-slate-200 dark:border-slate-800 px-3 py-2 text-right text-sm font-black">Certified Qty</th>
                          ...
                          <tbody>
                            {(selectedBill.lineItems || []).map((line, index) => (
                              <tr key={`${line.itemId}-${line.milestoneKey}`}>
                                ...
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-sm">{line.itemCode}</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-sm">{line.description}</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-center text-sm">{line.unit}</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-right text-sm">{formatQty(line.quantity)}</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-sm">{line.milestoneName}</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-center text-sm">{line.milestonePercentage}%</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-right text-sm">{formatMoney(line.rate)}</td>
                                ...
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-right text-sm font-black">{formatQty(line.currentQty)}</td>
                                <td className="border border-slate-200 dark:border-slate-800 px-3 py-3 text-right text-sm font-black">{formatMoney(line.currentAmount)}</td>
                      ...
                      <div className="rounded-[28px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 shadow-sm">
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white">Measurement Sheet Summary</h3>
                        <div className="mt-6 space-y-6">
                          {(selectedBill.measurementItems || []).map((item, itemIndex) => {
                            ...
                            return (
                              <div key={item.itemId} className="overflow-hidden rounded-[24px] border border-slate-900 dark:border-slate-700/80">
                                ...
                                <div className="overflow-x-auto">
                                  <table className="w-full border-collapse">
                                    <thead>
                                      <tr className="bg-slate-100 dark:bg-slate-900">
                                        <th className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center text-sm font-black">Sr.</th>
                                        {columns.map(([label]) => (
                                          <th key={label} className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center text-sm font-black">{label}</th>
                                        ))}
                                        <th className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-right text-sm font-black">100% Qty</th>
                                        {milestoneHeaders.map((entry) => (
                                          <th key={entry.milestoneKey} className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center text-sm font-black">
                                    ...
                                    <tbody>
                                      {item.measurements.map((row, rowIndex) => (
                                        <tr key={row.id}>
                                          <td className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center">{rowIndex + 1}</td>
                                          {columns.map(([label, key]) => (
                                            <td key={`${row.id}-${label}`} className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center">
                                              {getDisplayValue(row, key)}
                                            </td>
                                          ))}
                                          <td className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-right">{formatQty(Number(row.quantity || 0))}</td>
                                          {milestoneHeaders.map((entry) => {
                                            const milestoneQty = row.raBillMilestones.find((value) => value.milestoneKey === entry.milestoneKey)?.qty || 0;
                                            return (
                                              <td key={entry.milestoneKey} className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center font-bold">
                                      ...
                                      <tr className="bg-slate-100 dark:bg-slate-900">
                                        ...
                                        <td colSpan={Math.max(1, milestoneHeaders.length)} className="border border-slate-900 dark:border-slate-700/80 px-3 py-3 text-center text-lg font-black text-slate-900 dark:text-white">
                          ...
                          <div className="rounded-[24px] border border-slate-900 dark:border-slate-700/80 bg-slate-50 dark:bg-slate-900 p-5">
                            ...
                            <div className="mt-4 flex flex-wrap gap-12 text-lg">
                              <p className="text-slate-500">
                                Total Weight: <span className="font-black text-slate-900 dark:text-white">{formatQty(selectedBill.totalQty)} {selectedBill.unit}</span>
                              </p>
                              <p className="text-slate-500">
                                Bill Amount: <span className="font-black text-slate-900 dark:text-white">{formatMoney(selectedBill.totalAmount)}</span>
                              </p>
                            </div>
                          </div>

                          <div>
                            <p className="text-xl font-black text-slate-900 dark:text-white">Generated on: {formatDateTime(selectedBill.generatedAt)}</p>
                      ...
                      {(selectedBill.measurementItems || []).map((item, itemIndex) => {
                        ...
                        return (
                          <div key={item.itemId} className={itemIndex > 0 ? "mt-8" : ""}>
                            <div className="mb-2">
                              <p className="text-lg font-black text-slate-900 dark:text-white">{item.itemCode} - {item.description}</p>
                            </div>
                            <table className="w-full border-collapse">
                              <thead>
                                <tr>
                                  <th className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-center">Sr.</th>
                                  {columns.map(([label]) => (
                                    <th key={label} className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-center">{label}</th>
                                  ))}
                                  <th className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-right">Qty</th>
                                  {milestoneHeaders.map((entry) => (
                                    <th key={entry.milestoneKey} className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-center">
                                      {entry.percentage}% {entry.milestoneName}
                                    </th>
                                  ))}
                                </tr>
                              ...
                              <tbody>
                                {item.measurements.map((row, rowIndex) => (
                                  <tr key={row.id}>
                                    <td className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-center">{rowIndex + 1}</td>
                                    {columns.map(([label, key]) => (
                                      <td key={`${row.id}-${label}`} className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-center">
                                        {getDisplayValue(row, key)}
                                      </td>
                                    ))}
                                    <td className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-right">{formatQty(Number(row.quantity || 0))}</td>
                                    {milestoneHeaders.map((entry) => {
                                      const milestoneQty = row.raBillMilestones.find((value) => value.milestoneKey === entry.milestoneKey)?.qty || 0;
                                      return (
                                        <td key={entry.milestoneKey} className="border border-slate-900 dark:border-slate-700 px-2 py-2 text-center">
                                          {milestoneQty > 0 ? formatQty(milestoneQty) : "-"}
                                        </td>
                                      );
                                    })}
              ...
              <div className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-8 py-5">
                <div className="flex flex-wrap items-center justify-end gap-3">
                  <button
                    onClick={() => selectedBill && exportRABillToExcel(selectedBill)}
                    disabled={isLoadingDetail}
                    className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-5 py-3 text-sm font-black text-slate-900 dark:text-white transition-colors hover:bg-slate-50 dark:bg-slate-900 disabled:opacity-50"
                  >
